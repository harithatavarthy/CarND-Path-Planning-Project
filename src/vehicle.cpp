...
bool Vehicle::get_vehicle_behind(map<int, vector<Vehicle>> predictions, int lane, Vehicle & rVehicle) {
    ...
    for (map<int, vector<Vehicle>>::iterator it = predictions.begin(); it != predictions.end(); ++it) {
        temp_vehicle = it->second[it->first];
...
bool Vehicle::get_vehicle_ahead(map<int, vector<Vehicle>> predictions, int lane, Vehicle & rVehicle) {
    ...
    for (map<int, vector<Vehicle>>::iterator it = predictions.begin(); it != predictions.end(); ++it) {
        ...
        if (temp_vehicle.lane == lane && temp_vehicle.s > this->s && temp_vehicle.s < min_s && (temp_vehicle.s - this->s) < 100) {
...
vector<Vehicle> Vehicle::keep_lane_trajectory(map<int, vector<Vehicle>> predictions) {
    ...
    float new_d = (this->lane + 1)/2;
    trajectory.push_back(Vehicle(this->lane, new_s, new_d, new_v, new_a, "KL"));
    return trajectory;
}
...
vector<Vehicle> Vehicle::prep_lane_change_trajectory(string state, map<int, vector<Vehicle>> predictions) {
    ...
    trajectory.push_back(Vehicle(this->lane, new_s, new_d, new_v, new_a, state));
    return trajectory;
}
...
vector<Vehicle> Vehicle::lane_change_trajectory(string state, map<int, vector<Vehicle>> predictions) {
    ...
    for (map<int, vector<Vehicle>>::iterator it = predictions.begin(); it != predictions.end(); ++it)
    {
        next_lane_vehicle = it->second[it->first];
        if(this->v < 20.0)
        {
            return trajectory;
        }
        if (fabs(next_lane_vehicle.s - this->s) < 100 && next_lane_vehicle.lane == new_lane)
        {
            //If lane change is not possible, return empty trajectory.
            if(this->s > next_lane_vehicle.s && this->v < next_lane_vehicle.v)
            {
                return trajectory;
            }
            else if(next_lane_vehicle.s > this->s && this->v > next_lane_vehicle.v)
            {
                return trajectory;
            }
            else if(fabs(next_lane_vehicle.s - this->s) < 60)
            {
                return trajectory;
            }
        }
