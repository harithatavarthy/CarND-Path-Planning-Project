...
float inefficiency_cost(const Vehicle & vehicle, const vector<Vehicle> & trajectory, const map<int, vector<Vehicle>> & predictions, map<string, float> & data) {
    ...
    float proposed_speed_intended = lane_speed(predictions, data["intended_lane"],data["position"]);
    
    if (proposed_speed_intended < 0) {
        proposed_speed_intended = vehicle.target_speed;
    }
    
    float proposed_speed_final = lane_speed(predictions, data["final_lane"],data["position"]);
...
map<string, float> get_helper_data(const Vehicle & vehicle, const vector<Vehicle> & trajectory, const map<int, vector<Vehicle>> & predictions) {
    ...
    if (trajectory_last.state.compare("PLCR") == 0 && trajectory_last.lane < 2) {
        intended_lane = trajectory_last.lane + 1;
    } else if (trajectory_last.state.compare("PLCL") == 0 && trajectory_last.lane > 0) {
        intended_lane = trajectory_last.lane - 1;
    } else {
    ...
    trajectory_data["position"]=trajectory_last.s;
